...
output: times (TOTAL_WIDTH * TOTAL_HEIGHT) db 0
times TOTAL_HEIGHT db 0 ; it will also contain at least TOTAL_HEIGHT newline chars
db 0 ; null terminated with 0
  ...
  input_loop:
    ...
    .rotate_column:
      ...
      .rotate_col_loop:
        ...
        ; find the destination row: (row_i + shift_len) % TOTAL_HEIGHT
        mov rax, row_i
        add rax, shift_len

        mov rdx, 0
        mov rcx, TOTAL_HEIGHT
        div rcx
...
map_pixels_to_output_string:
  %define x_i r10
  %define y_i r11
  %define output_cursor r12

  mov output_cursor, 0

  mov x_i, 0
  .row_loop:
    ; throw in a newline on every row
    mov byte [output + output_cursor], 10
    inc output_cursor
    ...
    .column_loop:
      ...
      mul rdx
      add rax, x_i
      ; rax now contains (y * TOTAL_WIDTH) + x

      add rax, [this_screen]

      cmp byte [rax], 1
      jne .dot
      mov byte [output + output_cursor], '#'
      jmp .post_output
      .dot:
        mov byte [output + output_cursor], '.'
      .post_output:
