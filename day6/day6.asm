...
main:
  ...
  %define winner_char byte [rbp - 1] ; offset from 0 - 26
  %define winner_count word [rbp - 3] ; how many did we see?
  ...
  .continue_collect_loop:
    cmp cursor, 26 - 1
    jge .add_winner_to_output
    
    inc cursor
    jmp .collect_loop
  .add_winner_to_output:
    ; if there is no winner, just get out of here... we're past the columns from the input
    cmp winner_char, 0
    je .done_collect_loop

    movzx rax, winner_char
